...
#
# Tasks:
# 1. Signal Triage       — is this worth alerting or is it noise?
# 2. Dynamic Asset Map   — which assets are relevant given current regime?
# 3. Intelligence Brief  — write a genuine analyst-quality brief
# 4. Portfolio Awareness — what does this signal mean for open positions?
# 5. Exit Reasoning      — should we hold, exit, or tighten stop?
# 6. Track Record Docs   — document signal outcome when it expires
# 7. Morning Brief       — daily intelligence briefing at 9am ET
# 8. Overnight Monitor   — assess positions when breaking news fires at 3am
#
# Design principles:
# - Event-driven, NOT continuous — only runs when something needs reasoning
# - Tight context windows — only feeds what the agent needs for each task
# - Single Claude call per task — no chaining, no back-and-forth
# - All outputs stored in DB for audit trail
...
def get_open_positions():
    """Fetch open positions for portfolio-aware reasoning."""
    try:
        conn = get_db()
        cur = conn.cursor()
        cur.execute("""
...
def get_active_regime():
    """Get current macro regime from regime detector."""
    try:
        conn = get_db()
        cur = conn.cursor()
        cur.execute("""
...
def get_recent_signal_accuracy():
    """Get recent accuracy stats for context."""
    try:
        conn = get_db()
        cur = conn.cursor()
        cur.execute("""
...
def triage_signal(signal):
    """
    Decides if a signal is worth alerting or should be suppressed.
    Returns: 'alert' | 'suppress' | 'watch'
    """
    signal_id   = signal[0]
    ...
    system = (
        ...
        "You are evaluating whether a signal is worth alerting to the portfolio team. "
...
Should this signal be alerted to the team?
...
# ── Task 2: Dynamic Asset Mapping ─────────────────────────────────────────────

def dynamic_asset_map(signal_description, category, region, static_assets):
    """
    Given current macro regime, re-weights or reorders static asset mappings.
    Returns enriched asset list with agent reasoning.
    """
    regime = get_active_regime()
...
Re-rank these assets by relevance given the current regime.
...
def generate_brief(signal):
    """
    Writes a genuine analyst-quality intelligence brief.
    Replaces the templated brief with real reasoning.
    """
    description = signal[1] or ""
    region      = signal[2] or "Global"
    category    = signal[3] or ""
    ...
    top_assets = [f"{a.get('ticker')} ({a.get('direction')}, {a.get('avg_move_72h', 0):.1f}% avg 72h)"
                  for a in assets[:4]]
...
# ── Task 4 + 5: Portfolio Awareness + Exit Reasoning ─────────────────────────

def portfolio_signal_assessment(signal):
    """
    Assesses a signal against open positions.
    Returns specific guidance per position — hold, watch, exit, add.
    """
    description = signal[1] or ""
    region      = signal[2] or "Global"
    category    = signal[3] or ""
...
# ── Task 6: Track Record Documentation ───────────────────────────────────────
...
def document_outcome(signal_id, description, category, region,
    ...
    """
    When a signal expires, documents the outcome with narrative reasoning.
    Stores in signal_outcomes table as agent_narrative.
    """
...
def generate_morning_brief(signals, open_positions):
    """
    Replaces the template morning digest with a genuine intelligence brief.
    ...
    system = (
        ...
        "Write in clear professional prose. No bullet points in the main text. "
...
# ── Task 8: Overnight Monitor ─────────────────────────────────────────────────

def overnight_position_assessment(breaking_signal):
    """
    Called when a HIGH/EXTREME signal fires outside market hours.
    Assesses all open positions against breaking news.
    Returns urgent Telegram message.
    """
...
def run_agent_triage(signals):
    """
    Called by email_alert.py after get_unalerted_signals().
    Triages each signal — returns only the ones worth alerting.
    Enriches each with a brief and portfolio assessment.
    """
    if not signals:
        return []
    ...
    for signal in signals:
        ...
        # Generate brief for approved/watch signals
        brief = generate_brief(signal)

        # Portfolio assessment
        assessment = portfolio_signal_assessment(signal)

        # Attach to signal tuple as extra context
...
def run_agent_morning_brief(signals, open_positions):
    """
    Called by scheduler.py at 9am ET instead of template digest.
    """
...
def run_agent_outcome_documentation():
    """
    Called by scheduler.py hourly.
    Documents outcomes for expired signals that haven't been narrated yet.
    """
    ...
    try:
        ...
        for row in rows:
            ...
            # Get predicted direction from assets
